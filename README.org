...
Lisp callers that need to select an existing session without opening a frontend
can call =term-sessions-read-existing-session-entry=.  It requires a completion
match and returns the selected entry's name and location-aware directory,
including already-known TRAMP sessions.
