...
(defun term-sessions--open-eat (name command buffer-name &optional spec)
  ...
  (let ((buffer (eat-make (string-trim buffer-name "\\*" "\\*")
                          "/usr/bin/env" nil "sh" "-c" command)))
    (pop-to-buffer buffer)
    (with-current-buffer buffer
      (eat-char-mode)
      (term-sessions--mark-buffer name spec))))
...
`default-directory' values are handled by TRAMP or tramp-rpc."
  (unless (require 'eat nil t)
    (user-error "eat is not available"))
  (let ((buffer (apply #'eat-make (term-sessions--term-buffer-base-name name)
                       program nil args)))
    (pop-to-buffer buffer)
    (with-current-buffer buffer
      (eat-char-mode)
...
(defun term-sessions--open-tramp-process (name args frontend buffer-name &optional spec)
  "Attach to NAME by running zmx ARGS through TRAMP using FRONTEND."
  (term-sessions-zmx--with-environment
    (pcase frontend
      ('eat (term-sessions--open-eat-process name term-sessions-zmx-program args buffer-name spec))
      ('term (term-sessions--open-term-process name term-sessions-zmx-program args buffer-name spec))
      ...
      (pcase transport
        ('tramp-process
         (term-sessions--open-tramp-process
          name (term-sessions--attach-args name command) frontend buffer-name spec))
        ((or 'local 'ssh-wrapper)
         (pcase frontend
           ...
           ('term (term-sessions--open-term name attach-command buffer-name spec))
