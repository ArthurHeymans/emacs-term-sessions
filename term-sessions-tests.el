...
(ert-deftest term-sessions-test-remote-open-dispatches-local-ssh-wrapper-for-command-frontend ()
  (let ((default-directory "/ssh:user@example:/tmp/project")
    ...
    (cl-letf (((symbol-function 'term-sessions--ensure-zmx) #'ignore)
      ...
      (should (string-match-p "zmx\\\\ attach\\\\ dev\\\\ \\\\" (nth 1 opened)))
  ...
  (let ((default-directory "/ssh:user@example#2222:/tmp/project")
    ...
    (cl-letf (((symbol-function 'term-sessions--ensure-zmx) #'ignore)
              ...
              ((symbol-function 'term-sessions--open-tramp-process)
               (lambda (name args frontend buffer-name &optional _spec)
                 (setq opened (list name args frontend buffer-name default-directory)))))
      (term-sessions-open-with-frontend "dev" nil 'term nil)
      (should (equal opened
                     '("dev" ("attach" "dev") term "*term-session:dev: [ssh:user@example#2222] /tmp/project*"
                       "/ssh:user@example#2222:/tmp/project"))))))

(ert-deftest term-sessions-test-rpc-resolves-to-tramp-process-or-ssh-wrapper ()
...
(ert-deftest term-sessions-test-remote-interactive-attach-refuses-unsupported-method ()
  (let ((default-directory "/sudo:example:/tmp"))
    (should-error (term-sessions--ensure-interactive-attach-supported nil 'shell)
                  :type 'user-error)))
