...
(ert-deftest term-sessions-test-start-allows-create ()
  (let (opened)
    (cl-letf (((symbol-function 'term-sessions--ensure-zmx) #'ignore)
              ((symbol-function 'term-sessions--ensure-local-interactive-attach) #'ignore)
              ((symbol-function 'term-sessions--active-p) (lambda (_name) nil))
...
(ert-deftest term-sessions-test-remote-interactive-attach-refuses ()
  (let ((default-directory "/ssh:example:/tmp"))
    (should-error (term-sessions--ensure-local-interactive-attach)
                  :type 'user-error)))

(ert-deftest term-sessions-test-store-org-link-refuses-remote ()
  (let ((default-directory "/ssh:example:/tmp"))
    (should-error (term-sessions-store-org-link "dev")
                  :type 'user-error)))
